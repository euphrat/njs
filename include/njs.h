...
string DataTypeStrings [] = {"STACK", "INTEGER", "DOUBLE", "TEXT", "SP", "IF", "THEN", "ELSE", "RETURN", "TYPEOF"};
class Data{
private:
	int* refcount;
	void destroy(){
...
bool njs_private_func_eval(stack<Data>& s){
	if (!s.empty()){
		...
		switch(s.top().type)
		{
			...
			case TYPEOF: s.pop(); if(!s.empty()) {s.push(Data(new string(DataTypeStrings[s.top().type]), TEXT));} break;
